...
def update_data_json(institute_data, exam_data):
    try:
        ...
        # Update institute and exam details in JSON
...
def display_module(root):
    ...
    fields = [("Inst. Code of Exam Center", "INS_CODE"), 
...
def get_json_value(key, default_value="N/A"):
    try:
        with open('config/data.json', 'r') as f:
            data = json.load(f)
        return data.get('institute', {}).get(key, default_value)
    except (FileNotFoundError, json.JSONDecodeError):
        print(f"Error retrieving {key} from JSON.")
        return default_value

def get_json_date(var_name, default_date="2000-01-01"):
    date_str = get_json_value(var_name, default_date)
